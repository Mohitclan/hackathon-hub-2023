...
const Hero = () => {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e) => {
      ...
      if (hero) {
        ...
        // Apply more pronounced 3D effect
        hero.style.transform = `perspective(1200px) rotateY(${x * 5}deg) rotateX(${-y * 5}deg)`;
      }
    };
  ...
  return (
    ...
    >
      {/* Background imagery with 3D parallax */}
      <div className="absolute inset-0 z-0">
        <img 
          ...
          style={{ 
            transform: `translateZ(-10px) scale(1.1) translateX(${mousePosition.x * -15}px) translateY(${mousePosition.y * -15}px)`,
      ...
      {/* 3D Floating glowing effects */}
      <div className="absolute inset-0 z-0 overflow-hidden">
        <div 
          className="absolute w-[600px] h-[600px] rounded-full bg-marvel-blue/10 blur-[100px] -top-40 -left-40"
          style={{ 
            transform: `translateZ(-5px) translateX(${mousePosition.x * 20}px) translateY(${mousePosition.y * 20}px)`,
            transformStyle: 'preserve-3d'
          }}
        />
        <div 
      ...
      <div className="container mx-auto px-6 relative z-10 py-12 md:py-20" style={{ transformStyle: 'preserve-3d' }}>
          ...
          <div className="w-full md:w-1/2 md:pr-8" style={{ transform: 'translateZ(20px)', transformStyle: 'preserve-3d' }}>
            ...
            <Transition animation="fade-in-up" delay={300}>
              <h1 className="text-4xl md:text-7xl font-bold leading-tight mb-6 text-white" style={{ textShadow: '0 4px 8px rgba(0,0,0,0.3)' }}>
                <span className="block mb-2 text-marvel-red font-orbitron" style={{ textShadow: '0 0 10px rgba(211,47,47,0.3)' }}>HACKरण</span>
                <span className="block text-white text-5xl md:text-6xl" style={{ textShadow: '0 0 15px rgba(255,255,255,0.2)' }}>ASSEMBLE</span>
              </h1>
            </Transition>
          ...
          <div className="w-full md:w-1/2 mt-12 md:mt-0" style={{ transform: 'translateZ(30px)', transformStyle: 'preserve-3d' }}>
            <Transition animation="fade-in" delay={500} once={true}>
              <div className="relative" style={{ transformStyle: 'preserve-3d' }}>
                {/* Marvel-style 3D frame */}
                <div 
                  className="absolute inset-0 border-[3px] border-marvel-red rounded-lg" 
                  style={{ 
                    transform: `translateZ(5px) rotate(${2 + mousePosition.x * 5}deg)`,
                    boxShadow: '0 10px 30px -5px rgba(211, 47, 47, 0.3)',
                    transformStyle: 'preserve-3d'
                  }}
                ></div>
                <div 
                  className="absolute inset-0 border-[3px] border-marvel-blue rounded-lg"
                  style={{ 
                    transform: `translateZ(10px) rotate(${-1 + mousePosition.y * 5}deg)`,
                    boxShadow: '0 10px 30px -5px rgba(0, 123, 255, 0.3)',
                    transformStyle: 'preserve-3d'
                  }}
                ></div>
                ...
                <div 
                  className="relative z-10 overflow-hidden rounded-lg shadow-glow-strong"
                  style={{ 
                ...
                <div 
                  className="absolute -bottom-5 -right-5 bg-marvel-red text-white py-2 px-4 rounded font-orbitron shadow-glow z-20"
                  style={{ 
                    transform: `translateZ(25px) rotate(${3 + mousePosition.x * 2}deg)`,
                    transformStyle: 'preserve-3d'
                  }}
                >
                  JIMSEMTC
        ...
        <div 
          ...
          style={{ transform: `translateZ(15px) translateX(calc(-50% + ${mousePosition.x * 10}px))` }}
        >
          <a 
            href="#about" 
            className="flex items-center justify-center w-10 h-10 rounded-full bg-marvel-blue/20 shadow-glow hover:bg-marvel-blue/30 transition-colors duration-300 hover:shadow-glow-strong hover:scale-110"
